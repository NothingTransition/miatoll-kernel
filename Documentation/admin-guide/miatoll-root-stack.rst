...
The exact vendor boot-image, DTB/DTBO, AVB, and AnyKernel packaging steps are
platform-specific and are not generated by this source change.  Check the
resulting ``.config`` for the options above before flashing.  Keep a known-good
boot image and a way to disable KernelSU modules available.
